...
public class AlunoRepositoryImpl implements AlunoRepository {
    ...
    public Aluno save(facens.arquiteturaweb.aula3.modelo.Aluno aluno) {
        if (aluno.getId() != null) {
            String insertQuery = "INSERT INTO public.aluno (id, nome, curso, estadocivil, pronome, documento) VALUES (?, ?, ?, ?, ?, ?)";
            /*
             ...
             */
            jdbcTemplate.update(insertQuery, aluno.getId(), aluno.getNome(), aluno.getCurso(), aluno.getEstadoCivil(),
                    aluno.getPronome(), aluno.getDocumento());
        } else {
            String updateQuery = "UPDATE public.aluno SET nome = ?, curso = ?, estadocivil = ?, pronome = ?, documento = ? WHERE id = ?";
            jdbcTemplate.update(updateQuery, aluno.getNome(), aluno.getCurso(), aluno.getEstadoCivil(),
                    aluno.getPronome(),
                    aluno.getDocumento(), aluno.getId());
        }
        return aluno;
    }
}
